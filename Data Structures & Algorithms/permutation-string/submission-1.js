class Solution {
    ...
    checkInclusion(s1, s2) {
        ...
        const firstCharacter = s1[0];

        for (let a = 0; a < s2.length; a++) {
            if (s2[a] === firstCharacter) {
                let left = Math.max(a - s1.length + 1, 0);
                let right = Math.min(a + s1.length - 1, s2.length - 1);
                possibleWindows.push([left, right]);
            }
        ...
        for (const window of possibleWindows) {
            let l = window[0], r = window[0];
